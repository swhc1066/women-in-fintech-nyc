 ...
 *
 * Ported verbatim from src/admin/templates.js, which generates a post in the
 * browser. The build now renders posts from data, so the same text would be
 * escaped twice over by two copies of this logic — and they would drift. This
 * module is the single source; the admin imports it once Phase 5 rewrites the
 * editor, and templates.js loses its copy then.
 *
...
export function renderInline(text) {
  ...
  out = out.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, label, url) => {
    ...
    const raw = String(url).replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
