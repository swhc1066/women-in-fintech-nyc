...
/* JSON string syntax is valid YAML, so single-line values go out as-is.
   ...
   A `|-` block strip-chomps trailing blank lines (gray-matter, the reader
   Eleventy actually builds the site with, chomps the same way, so a trailing
   newline would come back truncated on both sides alike) and leading spaces
   on the first line are read as the block's indentation, which breaks when a
   later line is indented less -- gray-matter then throws instead of
   rendering wrong, and a malformed post must not take the build down. Any
   value with leading or trailing whitespace goes out as a JSON string
   instead, which both parsers read back losslessly. */
export function yamlValue(value, indent) {
  const s = String(value == null ? '' : value);
  if (!s.includes('\n') || s.trim() !== s) return JSON.stringify(s);
